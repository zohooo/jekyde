...
var sitedata = {title: 'Simple Blog', posts: [], pages: []};
var templates = {}
...
function readArticles(type) {
    ...
    for (i = 0; i < files.length; i++) {
        ...
        addSiteData(type, '/' + type + '/', name, ext);
    }

    function addSiteData(type, path, name, ext) {
        var text = fs.readFileSync(srcdir + path + name + ext, {encoding: 'utf8'});
...
function parseMark(type, item) {
    ...
    fs.writeFileSync(webdir + item.url, html);
}
