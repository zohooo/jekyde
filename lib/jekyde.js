...
function initialize() {
    if (!fs.existsSync(cdir)) {
        console.log('Initializing default content folder...');
        fsextra.copy((path.normalize(__dirname + '/../sample/content')), cdir, function(err){
            if (err) throw err;
            loadConfig();
            generate();
        });
...
function copyStatic() {
    if (fs.existsSync(tdir + '/static')) {
        fsextra.copy(tdir + '/static', wdir + '/static', function(err){
            if (err) throw err;
