...
function pushData(type, basename, text) {
    var list = sitedata[type + 's'];
    var obj = parseYaml(text);
    if (type == "post") {
        parseDate(obj, basename);
        if (isNaN(obj.date.getTime())) {
            console.log('[Error] Invalid filename "' + basename + '.md"');
            return;
        }
        if (list.length > 0) {
...
function sortPosts() {
    ...
    sitedata['posts'].sort(comparePosts);
}
...
function update(change) {
    switch(change.mode) {
        case 'create':
            pushData(change.type, change.name, change.text);
            if (change.type == 'post') sortPosts();
            break;
        case 'modify':
            break;
        case 'rename':
